...
fn match_pattern(input: &str, pattern: &str) -> bool {
    ...
    let mut had_match = consume_if_starts_with(&mut pattern, b"^");
    while pattern.len() > 0 {
        let Some(at) = input.next() else {
            return false;
        };
        let mut old_pattern = pattern;
        let old_pattern = pattern;
        let matched;
        if consume_if_starts_with(&mut pattern, b"\\d") {
            matched = at.is_ascii_digit();
        } else if consume_if_starts_with(&mut pattern, b"\\w") {
            matched = at.is_ascii_alphanumeric() || at == b'_';
        } else if consume_if_starts_with(&mut pattern, b"[") {
            let negative = consume_if_starts_with(&mut pattern, b"^");
            let mut chars = vec![];
            while !consume_if_starts_with(&mut pattern, b"]") {
                chars.push(consume(&mut pattern));
            }
            matched = negative ^ chars.contains(&at);
        } else {
            let expected = consume(&mut pattern);
            matched = at == expected;
        }
        if !matched {
            if had_match {
    ...
    #[inline]
    fn consume_if_starts_with(str: &mut &[u8], pat: &[u8]) -> bool {
...
fn main() {
    ...
    std::process::exit(!match_pattern(&input_line, &pattern) as i32)
}
